...
def main():
    ...
    print(f"Box Precision:   {val_results.box.p[0]:.4f}" if hasattr(val_results.box.p, '__len__') else f"Box Precision:   {val_results.box.p:.4f}")
    print(f"Box Recall:      {val_results.box.r[0]:.4f}" if hasattr(val_results.box.r, '__len__') else f"Box Recall:      {val_results.box.r:.4f}")
    print()
    print(f"Mask mAP50:      {val_results.seg.map50:.4f}")
    print(f"Mask mAP50-95:   {val_results.seg.map:.4f}")
    print(f"Mask Precision:  {val_results.seg.p[0]:.4f}" if hasattr(val_results.seg.p, '__len__') else f"Mask Precision:  {val_results.seg.p:.4f}")
    print(f"Mask Recall:     {val_results.seg.r[0]:.4f}" if hasattr(val_results.seg.r, '__len__') else f"Mask Recall:     {val_results.seg.r:.4f}")
    print("="*50)
    print()
